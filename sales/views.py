...
def DashboardView(request):
    if request.session.has_key('user'):
        ...
        cantidad_total = 0
        try:
            top_sales = SalesDetails.objects.filter(fecha = date.today())
        ...
        df_sales['fecha'] = pd.to_datetime(df_sales['fecha'])
        df_sales = df_sales.groupby('fecha').cantidad.sum().reset_index()
        # print(df_sales)
        x_data =df_sales['fecha']
        y_data = df_sales['cantidad']
        ...
        df_diff['prev_sales'] = df_diff['cantidad'].shift(1)
        df_diff = df_diff.dropna()
        df_diff['diff'] = (df_diff['cantidad'] - df_diff['prev_sales'])
        # print(df_diff.head(5))
        plot_differ = plot([Scatter(x=df_diff['fecha'], y=df_diff['diff'],
        ...
        df_model = df_supervised.drop(['cantidad','fecha'],axis = 1)
        # print("df_model", df_model)
        train_set, test_set = df_model[0:-6].values, df_model[-38:].values
        ...
        print("trainsetscaled", train_set_scaled)
        ...
        result_list=[]
        sales_dates = list(df_sales[-39:].fecha)
        act_sales = list(df_sales[-39:].cantidad)
        ...
        print("df_sales", df_sales_pred)
        print("df_result", df_result)
        pred_plot = plot([Scatter(x=df_sales_pred['fecha'], y=df_sales_pred['cantidad'],
                        mode='lines', name='actual',
                         marker_color='green'),
                         Scatter(x=df_sales_pred['fecha'], y=df_sales_pred['pred_value'],
        ...
        for i in total_sales:
            total += i.cantidad
        return render(request,'dashboard.html',{'cantidad_total':cantidad_total,'pred_plot':pred_plot,'user':user,'diff_plot':plot_differ,'count':product_count,'top':top_sales,'total':total,'plot_div': plot_div})
    else:
        return redirect('login')
...
def UrbanizacionView(request):
    data = {}
    return render(request,'urbanizacion.html')

def TerrenoView(request):
    data = {}
...
def ProductView(request):
    if request.session.has_key('user'):
        ...
        if request.POST:
            ...
            model.urbanizacion = request.POST['product']
            model.save()
...
def SalesView(request):
    if request.session.has_key('user'):
        ...
        sale_details = SalesDetails.objects.filter(fecha = date.today())
        page = request.GET.get('page', 1)
        ...
        if request.POST:
            model = SalesDetails()
            model.urbanizacion = request.POST['urbanizacion']
            model.fecha = request.POST['fecha']
            model.cantidad = request.POST['cantidad']
            model.save()
            print(model.id)
